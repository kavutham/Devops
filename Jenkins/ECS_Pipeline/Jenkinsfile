...
pipeline {
    ...
    stages {
        ...
        stage('Build Docker Image') {
            steps {
                withCredentials([string(credentialsId: 'cloud_user', variable: 'AWS_ECR_URL')]) {
                    script {
        ...
        stage('Deploy in ECS') {
            steps {
                withCredentials([string(credentialsId: 'cloud_user', variable: 'AWS_ECS_EXECUTION_ROL'),string(credentialsId: 'cloud_user', variable: 'AWS_ECR_URL')]) {
                    script {
